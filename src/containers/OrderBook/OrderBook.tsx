...
const OrderBook = () => {
  ...
  const {asks, bids, highestBid, lowestAsk, parseData, clearData } = useBookData();
  const [contractType, setContractType] = useState<OrderBookContractType>(OrderBookContractType.XBT);

  const handleMessage = useCallback((message: Event) => {
    parseData(message);
  }, [parseData]);
  ...
  const unsubscribe = () => {
    if (socket && socket.readyState === WebSocket.OPEN) {
      socket.send(JSON.stringify(buildOpenMessage(FeedEvent.UNSUBSCRIBE, FeedTypes.BOOK, [contractType])));
    };
  };

  const subscribe = useCallback(() => {
    if (socket && socket.readyState === WebSocket.OPEN) {
      socket.send(JSON.stringify(buildOpenMessage(FeedEvent.SUBSCRIBE, FeedTypes.BOOK, [contractType])));
    };
  }, [socket, contractType]);
  ...
  const handleContractToggle = () => {
    clearData();
    unsubscribe();
    setContractType(type => type === OrderBookContractType.ETH ? OrderBookContractType.XBT : OrderBookContractType.ETH);
  }
